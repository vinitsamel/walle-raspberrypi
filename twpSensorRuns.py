...
def sendEmailAlert(claimNum):
    msg = MIMEText("Collision Detected on Raspberry Pi!")
    me  = 'emailhere@??.com'
    you = 'phonenum'
    msg['Subject'] = 'Collision Alert : ClaimNum {0}'.format(claimNum)
    msg['From'] = me
    msg['To'] = you

    # Send the message via our own SMTP server, but don't include the
    # envelope header.
    s = smtplib.SMTP('smtp.gmail.com', 587, 'theweekendproject')
    s.set_debuglevel(1)
    s.ehlo()
    s.starttls()
...
def prepFNOL():
    claimNum = '{0:06}-01'.format(random.randint(1, 100000))
